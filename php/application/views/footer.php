  
  <script src="<?= base_url('static/node_modules/jquery/dist/jquery.js'); ?>"></script>
  <script src="<?= base_url('static/node_modules/bootstrap/dist/js/bootstrap.js'); ?>"></script>
  <script src="<?= base_url('static/node_modules/moment/min/moment.min.js'); ?>"></script>
  <script src="<?= base_url('static/node_modules/jquery-cycle2/build/jquery.cycle2.min.js'); ?>"></script>
  <script src="<?= base_url('static/node_modules/jquery-cycle2/build/plugin//jquery.cycle2.scrollVert.min.js'); ?>"></script>
  <script src="<?= base_url('static/node_modules/datatables/media/js/jquery.dataTables.min.js'); ?>"></script>

</body>
</html>
